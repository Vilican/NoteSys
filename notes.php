...
echo '<a href="logout.php">'.$logout.'</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="addnote.php">'.$addnote.'</a>';
if ($_SESSION["id"] == 0) { echo '&nbsp;&nbsp;&nbsp;&nbsp;<a href="users.php">'.$users.'</a>'; } else { echo '&nbsp;&nbsp;&nbsp;&nbsp;<a href="chpass.php">'.$chpass.'</a>'; }
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        if (($row["autor"] == $_SESSION["id"]) or ($_SESSION["id"] == 0) or ($isadmin == 1)) {
			echo "<p align='center'><a href='delete.php?id=". $row["id"] ."'>". $delete ."</a> - <a href='edit.php?id=". $row["id"] ."'>". $edit ."</a></p><p align='center'>". $date . " " . $row["date"]. "</p><p align='center'>". $field . " " . $row["value"]. "</p><br>";
		} else {
